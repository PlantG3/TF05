#!/bin/sh
#SBATCH --job-name=np1run
#SBATCH --output=../cache/03_npslurm.out
...
# input
########################################
ref=${mypath}/cache/02_${out}.asm${asm_version}/${out}.${asm_version}.contigs.fasta
...
# these parameters are set in 00_parameters.sh
#np_prefix=np
#np_ncpu=8
#np_mem_per_cpu=6g
...
echo "2b. index reads for nanopolish" &>>$np_log
...
echo "2c. run NP correction" &>>$np_log
cd $correctDir
